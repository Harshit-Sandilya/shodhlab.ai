...
const JobCard: FC<DocumentData> = ({ job: { title, location, slug } }) => {
	return (
		<div className='flex flex-col justify-between w-full gap-4 p-4 text-white bg-white lg:p-8 lg:items-center lg:flex-row bg-opacity-20 rounded-xl'>
			...
			<Link
				href={`jobs/${slug}`}
				className='blocpx-6 lg:px-8 py-[9px] lg:py-3 text-sm lg:text-base text-white transition duration-200 rounded-full border border-white hover:shadow-xl text-center'
			>
				Apply Now
			</Link>
		</div>
